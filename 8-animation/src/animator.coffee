...
define ['jquery', 'constants', 'log', 'canvas'], ($, constants, log, canvas) ->
  class Animator
    constructor: ->
      @lockFPS        = true
      @running        = true
    ...
    removeListener: (fn) =>
      if @performing
        @opCache.push => @removeListener(fn)
      else
    ...
    performLoop: =>
      if @running
        ...
        l(start - @last) for l in @listeners
        canvas.redraw()
