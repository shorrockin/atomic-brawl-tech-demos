...
define ['jquery', 'log', 'constants', 'utils/HashMap', 'ui/LayerList', 'ui/DirtyList', 'ui/Rectangle'], ($, log, constants, HashMap, LayerList, DirtyList, Rectangle) ->
  canvas            = $('#canvas')[0]
        ...
        layers.each (layer) ->                                          # iterate over each layer in this linked list
          if layer.enabled() and layer.bounds.intersects(canvas.bounds) # is this viewable and enabled?
            canvas.dirty.each (rectangle) ->                            # iterate through all the dirty rectangles
              unless drawn.exists(layer.name)                           # have we already drawn it due to a previous iteration?
               if layer.bounds.intersects(canvas.bounds)                # is this layer viewable?
                 if rectangle.intersects(layer.bounds)                  # does this layer intersect our dirty region (tee hee hee)
                  drawn.add(layer.name, true)                           # remember we've already drawn this
