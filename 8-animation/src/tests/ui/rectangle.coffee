define ['tests/assert', 'ui/Rectangle', 'log'], (assert, Rectangle, log) ->
  return {
    ...
    angularIntersction: ->
      ...
      assert.ok r2.intersects(r1), "r2 did not intersect r1"
    ...
    sameIntersection: ->
      ...
      assert.ok r1.intersects(r1), "r1 does not intersect itself"
    ...
    doesntIntersect: ->
      ...
      assert.fail base.intersects(left),  "base intersects left"
    ...
    rectangleContains: ->
      ...
      assert.fail r2.contains(r1), "r2 contained r1"
    ...
    rectangleMerging: ->
      ...
      assert.ok r2.merge(r1).equals(m), "merged r2 with r1 rectangle did not match the original #{m} and #{r2.merge(r1)}"

    rectangleContraction: ->
      r1 = new Rectangle 10, 10, 10, 10
      r2 = new Rectangle 13, 13, 15, 15
      c  = r1.contract(r2)

      assert.ok c.equals(new Rectangle(13, 13, 10, 10)), "contracted rectangle did not match expected"
      assert.ok r2.contract(r1).equals(c), "contract r2 with r1 rectangle did not match the original #{c} and #{r2.contract(r1)}"
